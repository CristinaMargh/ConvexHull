...
/* thread-local midpoint used by qsort comparator */
...
/* ---------- Divide & Conquer (MPI local) with Pthreads ---------- */
...
static Pair *divide_serial(Pair *a, int n, int *ret_size) {
    if (n <= BRUTE_THRESHOLD) return bruteHull(a, n, ret_size);

    int mid = n / 2;
    int ls, rs;
    Pair *L = divide_serial(a, mid, &ls);
    Pair *R = divide_serial(a + mid, n - mid, &rs);

    Pair *res = merger(L, ls, R, rs, ret_size);
    free(L);
    free(R);
    return res;
}

static Pair *divide_pth(Pair *a, int n, int depth, int max_depth, int *ret_size);

static void *thread_entry(void *arg) {
    ThreadJob *job = (ThreadJob *)arg;
    *(job->out_hull) = divide_pth(job->a, job->n, job->depth, job->max_depth, job->out_size);
    return NULL;
}

static Pair *divide_pth(Pair *a, int n, int depth, int max_depth, int *ret_size) {
    if (n <= BRUTE_THRESHOLD) return bruteHull(a, n, ret_size);
    if (depth >= max_depth)   return divide_serial(a, n, ret_size);
    ...
    /* spawn 1 thread for left, compute right in current thread */
    if (pthread_create(&th, NULL, thread_entry, &left_job) != 0) {
        /* fallback: no thread, do serial on left */
        left_hull = divide_pth(a, mid, depth + 1, max_depth, &left_size);
    }

    right_hull = divide_pth(a + mid, n - mid, depth + 1, max_depth, &right_size);

    if (left_hull == NULL) {
        pthread_join(th, NULL);
    } else {
        /* if we didn't create thread, nothing to join */
        if (left_hull != NULL && left_size >= 0) {
        }
    }
...
static int max_depth_from_threads(int threads) {
...
int main(int argc, char **argv) {
    ...
    /* local hull  Pthreads */
    int threads = 4;
    const char *env = getenv("PTH");
    if (env) {
        int v = atoi(env);
        if (v > 0) threads = v;
    }
    int max_depth = max_depth_from_threads(threads);

    int local_hull_size = 0;
    Pair *local_hull = divide_pth(local_points, local_n, 0, max_depth, &local_hull_size);
    free(local_points);
